...
class DataReader:
    ...
    def __init__(self, cif_dir, random_seed=123):
        self.cif_dir = cif_dir
        assert os.path.exists(cif_dir), 'cif_dir does not exist!'
        id_prop_file = os.path.join(self.cif_dir, 'id_prop.csv')
        ...
        for suffix in ['.cif', '.xyz', '.mol', '.pdb', '.sdf']:
            if os.path.exists(os.path.join(self.cif_dir, material_id + suffix)):
                self.suffix = suffix
                break
    ...
    def get_structure_by_id(self, material_id):
        structure = None
        if self.suffix == '.cif':
            structure = Structure.from_file(os.path.join(self.cif_dir, material_id + self.suffix))
        elif self.suffix == '.mol' or self.suffix == '.xyz' or self.suffix == '.pdb' or self.suffix == '.sdf':
            structure = IMolecule.from_file(os.path.join(self.cif_dir, material_id + self.suffix))
        return structure
...
class BondFeatureEncoder(object):
    ...
    def __init__(self, max_num_nbr=12, radius=5, dmin=0, step=0.1):
...
class GraphData(Dataset):
    ...
    def __init__(self, cif_dir, filename, max_num_nbr=12, radius=5, dmin=0, step=0.2,
                 random_seed=123, properties_list=None):
        self.data_reader = DataReader(cif_dir=cif_dir, filename=filename, random_seed=random_seed)
