...
from utils import Normalizer, AverageMeter, mae_metric, class_metric, Log
...
parser.add_argument('filename', metavar='F', help='csv filename(dataset/prop/filename.csv)')
...
def main():
    ...
    dataset = GraphData(cif_dir=path, filename=args.filename, max_num_nbr=model_args.max_num_nbr, radius=model_args.radius,
...
def test(test_loader, model, criterion, normalizer):
    ...
    test_feature_after_pooling = []
    test_feature_before_out = []
    ...
    for batch_idx, data in enumerate(test_loader, 0):
        with torch.no_grad():
            ...
            feature_after_pooling = model.feature_after_pooling
            feature_before_out = model.feature_before_out
            ...
            test_feature_after_pooling += feature_after_pooling.tolist()
            test_feature_before_out += feature_before_out.tolist()
    ...
    # ---------------------------------------------------------------------
    with open('results/feature_after_pooling.csv', 'w') as f:
        writer = csv.writer(f)
        for material_id, target, feature_after_pooling in zip(test_material_ids, test_targets, test_feature_after_pooling):
            writer.writerow((material_id, target, feature_after_pooling))

    df = pd.read_csv('results/feature_after_pooling.csv',
                     header=None,
                     names=['material_id', 'Target', 'feature_after_pooling'])
    df.to_csv('results/feature_after_pooling.csv', index=False)

    with open('results/feature_before_out.csv', 'w') as f:
        writer = csv.writer(f)
        for material_id, target, feature_before_out in zip(test_material_ids, test_targets, test_feature_before_out):
            writer.writerow((material_id, target, feature_before_out))

    df = pd.read_csv('results/feature_before_out.csv',
                     header=None,
                     names=['material_id', 'Target', 'feature_before_out'])
    df.to_csv('results/feature_before_out.csv', index=False)
